...
-- 1. ADIM: VERÝLERÝ GEÇÝCÝ TABLOYA HAZIRLAMA
IF OBJECT_ID('tempdb..#SatisRaporu') IS NOT NULL DROP TABLE #SatisRaporu;


-- 1. ADIM: VERÝLERÝ GEÇÝCÝ TABLOYA HAZIRLAMA
IF OBJECT_ID('tempdb..#SatisRaporu') IS NOT NULL DROP TABLE #SatisRaporu;
-- Geçici tabloyu oluþtur ve verileri ekle
SELECT * INTO #SatisRaporu FROM (
    SELECT 
        TARÝH,
        ISNULL(FORMAT([TÜRKÝYE], 'N2', 'tr-TR') + ' TL', 'SATIS YOK') AS TR,
        ISNULL(FORMAT([ULKE1], 'N2', 'tr-TR') + ' TL', 'SATIS YOK') AS ULKE1,
        ISNULL(FORMAT([ULKE2], 'N2', 'tr-TR') + ' €', 'SATIS YOK') AS KSV
    FROM (
        SELECT * FROM (
            -- TÜRKÝYE
            SELECT 'TÜRKÝYE' AS 'ULKE', CONVERT(VARCHAR(10), T.TRANS_DATE, 104) 'TARÝH',               
            ...
            UNION ALL
            -- ULKE1
            SELECT 'ULKE1' AS 'ULKE', CONVERT(VARCHAR(10), T.TRANS_DATE, 104) 'TARÝH',               
                   SUM(CASE WHEN PTYPE <> 2 THEN 1 ELSE -1 END * (GROSS_TOTAL-(DISCOUNT_ON_LINES+DISCOUNT_ON_TOTAL))) 'NETSATIS' 
            ...
            UNION ALL
            -- ULKE2
            SELECT 'KOSOVA' AS 'ULKE', CONVERT(VARCHAR(10), T.TRANS_DATE, 104) 'TARÝH',             
                   SUM(CASE WHEN PTYPE <> 2 THEN 1 ELSE -1 END * (GROSS_TOTAL-(DISCOUNT_ON_LINES+DISCOUNT_ON_TOTAL))) 'NETSATIS' 
        ...
        (
            SUM(NETSATIS)
            FOR ULKE IN ([TÜRKÝYE], [ULKE1], [ULKE2])
        ) AS PIVOTTABLO
    ) AS SonucTablosu
) AS TempData;
