...
SELECT * INTO #SatisRaporu FROM (
    SELECT 
        ...
        ISNULL(FORMAT([ULKE2], 'N2', 'tr-TR') + ' €', 'SATIS YOK') AS ULKE2
    FROM (
        SELECT * FROM (
            -- TÜRKiYE
            ...
            UNION ALL
            -- ULKE1
            SELECT 'ULKE1' AS 'ULKE1', CONVERT(VARCHAR(10), T.TRANS_DATE, 104) 'TARiH',               
                   SUM(CASE WHEN PTYPE <> 2 THEN 1 ELSE -1 END * (GROSS_TOTAL-(DISCOUNT_ON_LINES+DISCOUNT_ON_TOTAL))) 'NETSATIS' 
            ...
            UNION ALL
            -- ULKE2
            SELECT 'KOSOVA' AS 'ULKE2', CONVERT(VARCHAR(10), T.TRANS_DATE, 104) 'TARiH',             
                   SUM(CASE WHEN PTYPE <> 2 THEN 1 ELSE -1 END * (GROSS_TOTAL-(DISCOUNT_ON_LINES+DISCOUNT_ON_TOTAL))) 'NETSATIS' 
